...
//			}
	...
	static int getOnBlock(final Location l) {
		...
		if (id == 0 && Math.abs((l.getY() - l.getBlockY()) - 0.5) < Skript.EPSILON) { // fences
			...
			if (id != Material.LEGACY_FENCE.getId() && id != 107 && id != 113) // fence gate // nether fence
				return 0;
		}
		return id;
	}
	...
	static int getBlockY(final double y, final int id) {
		if ((id == Material.LEGACY_FENCE.getId() || id == 107 || id == 113) && Math.abs((y - Math.floor(y)) - 0.5) < Skript.EPSILON) // fence gate // nether fence
