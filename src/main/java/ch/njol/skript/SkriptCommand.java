	...
	public List<String> onTabComplete(CommandSender sender, Command cmd, String s, String[] args) {
		...
		if (listFiles != null) {
			...
			if (!sender.isOp()) {
				...
				if (args.length == 1) {
					StringUtil.copyPartialMatches(args[0], command, list);
				} else {
					switch (args[0]) {
						case "reload":
							for (File f : files) {
								if (f != null && !f.isDirectory() && !f.getName().startsWith("-") && f.getName().endsWith(".sk")) {
						...
						case "disable":
							...
							break;
						case "update":
							StringUtil.copyPartialMatches(args[1], update, list);
							break;
					}
				}
				return list;
