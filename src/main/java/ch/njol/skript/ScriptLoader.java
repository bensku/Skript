...
import ch.njol.skript.scopes.Repeated;
...
final public class ScriptLoader {
    ...
    @Nullable
    private static SectionNode currentNode;
    ...
    public static SectionNode getCurrentNode() {
        return currentNode;
    }
        ...
        for (final Node n : node) {
            ...
            if (n instanceof SimpleNode) {
                ...
                if (!SkriptParser.validateLine(name))
                    continue;
                TriggerSection section = SkriptParser.parse(name, Skript.getScopes().iterator(), "Can't understand this scope : " + name);
                final Kleenean hadDelayBefore = hasDelayBefore;
                items.add(section);
                hasDelayBefore = section instanceof Repeated && (hadDelayBefore != Kleenean.TRUE && hasDelayBefore != Kleenean.FALSE) ?
                        Kleenean.UNKNOWN :
                        hadDelayBefore.or(Kleenean.UNKNOWN);
            }
        }
