...
final public class ScriptLoader {
	...
	private final static HashMap<String, String> currentOptions = new HashMap<>();
...
//	}
	 ...
	 */
	private final static FileFilter scriptFilter = file ->
			file != null && (file.isDirectory() || StringUtils.endsWithIgnoreCase("" + file.getName(), ".sk")) && !file.getName().startsWith("-");
...
//			final SerializedScript script = new SerializedScript();
					...
					if (event.equalsIgnoreCase("aliases")) {
						...
						for (final Node n : node) {
							...
							if (t == null)
								continue;
							String key = n.getKey();
							assert key != null; // Eclipse
							currentAliases.put(key.toLowerCase(), t);
						}
						continue;
						...
						for (final Node n : node) {
							...
							String key = n.getKey();
							assert key != null; // Eclipse
							String name = key.toLowerCase(Locale.ENGLISH);
							if (name.startsWith("{") && name.endsWith("}"))
								name = "" + name.substring(1, name.length() - 1);
							final String var = name;
							name = StringUtils.replaceAll(name, "%(.+)?%", matcher -> {
								...
								final ClassInfo<?> ci = Classes.getClassInfoFromUserInput("" + matcher.group(1));
								if (ci == null) {
	...
	public static String replaceOptions(final String s) {
		final String r = StringUtils.replaceAll(s, "\\{@(.+?)\\}", matcher -> {
			final String option = currentOptions.get(matcher.group(1));
			if (option == null) {
