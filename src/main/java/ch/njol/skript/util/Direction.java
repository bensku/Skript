...
public class Direction implements YggdrasilRobustSerializable {
	...
	public static Location[] getRelatives(final Block[] blocks, final Direction[] directions) {
		...
		for (int i = 0; i < blocks.length; i++) {
			...
			for (Direction direction : directions) {
				r[i].add(direction.getDirection(blocks[i]));
			}
		}
		return r;
	}
	...
	public static Location[] getRelatives(final Location[] locations, final Direction[] directions) {
		...
		for (int i = 0; i < locations.length; i++) {
			...
			for (Direction direction : directions) {
				r[i].add(direction.getDirection(locations[i]));
			}
		}
		return r;
	}
...
//				return dirs.isSingle() && locs.isSingle();
	...
	@Override
	public boolean incompatibleField(@NonNull final Field f, @NonNull final FieldContext value) {
		return false;
	}
	...
	@Override
	public boolean missingField(@NonNull final Field field) {
