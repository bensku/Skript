...
public class EndermanData extends EntityData<Enderman> {
	...
	public boolean match(final Enderman entity) {
		...
		return hand == null || SimpleExpression.check(hand,
				t -> t != null && t.isOfType(entity.getCarriedMaterial().getItemTypeId(), entity.getCarriedMaterial().getData()),
				false, false
		);
	}
...
//		return b.toString();
	...
	private boolean isSubhand(final @Nullable ItemType[] sub) {
		if (hand != null) // shortened version errors with Eclipse compiler
	...
	public boolean isSupertypeOf(final EntityData<?> e) {
		return e instanceof EndermanData && isSubhand(((EndermanData) e).hand);
	}
